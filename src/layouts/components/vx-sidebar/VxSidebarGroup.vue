<!-- =========================================================================================
	...
	Author: Pixinvent
	Author URL: http://www.theme
    
    est.net/user/pixinvent
========================================================================================== -->
  ...
  <div
	...
	<div @click="clickGroup" class="group-header w-full">
		...
		<span class="flex items-center w-full">
            ...
            <span v-if="group.userMenu" v-show="!sidebarItemsMin" class="truncate mr-3">{{activeUser.name}}<small> ({{activeUser.level}})</small></span>
			<span v-else v-show="!sidebarItemsMin" class="truncate mr-3">{{ group.name }}</span>
    ...
    <ul ref="items" :style="styleItems" class="vs-sidebar-group-items">
      ...
      <li v-for="(groupItem, index) in group.submenu" :key="index">
		...
		<vx-sidebar-item :index="groupIndex + '.' + index" :to="groupItem.url" :icon="itemIcon(groupIndex + '.' + index)" icon-small :target="groupItem.target" v-else>
			...
			<span v-if="groupItem.name.slice(0,9) == 'category='" class="truncate">{{ $store.state.categories1.find(category => category.code == groupItem.name.split('=')[1]).name }} 배치</span>
            <span v-else class="truncate">{{ groupItem.name }}</span>
...
export default {
    ...
    watch: {
        activeUserName(){
            this.activeUser = this.activeUserInfo;
        },
    ...
    mounted() {
        ...
        // console.log(this.$store.state.categories1);
    },
}
</script>
